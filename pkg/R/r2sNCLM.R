...
r2sCLM = proto(
  ...
  estimate = function(.) {
   ...
   .$estimated = FALSE
    ...
    # Estimate
    cmd = paste(".$Rmodel <-",.$func,"(",dv,"~1,nominal=",.$ivField,",link='",link,"',data=",.$data,",subset=",.$subset,",weights=",.$weights,")",sep="")
  ...
  getFactorList = function(.) {
    varTypes = attr(attr(.$Rmodel,"terms"),"dataClasses")
